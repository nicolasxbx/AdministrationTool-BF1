...
{    
    ...
    public static async Task LogWinSwitchKick(BreakRuleInfo info, ChangeTeamInfo info2)
    {        
        string begin = $"✅WS-KICK\n";
        
        string name = $"    Name: {info.Name}\n";
        string kick_reason = $"    Kick Reason: {TBold(info.Reason)}\n";
        string status = $"    Info: {info2.Status}\n";
        string scores = $"    Scores: {info2.Team1Score} - {info2.Team2Score}\n";
        string map = $"    Map: {Vari.CurrentMapName}\n";
        string pid = $"    PID: {info.PersonaId}\n";
    ...
    public static async Task LogBalancer(ChangeTeamInfo info)
    {
        ...
        if (Vari.Server_AdminList_PID.Contains(info.PersonaId) == true)
        {
        ...
        {
            if (PlayerUtil.CheckAdminVIP(info.PersonaId, Vari.Server_VIPList) == "✔")
            {
